...
var FarmWork = {
  ...
  hayStorageBlip: null,
  HayCollector: function HayCollector() {
    var number = this.getRandomInt(this.hayPositions.length);
  ...
  tractorRoute: [{
    ...
    y: 5115.87548828125,
    z: 46.56815719604492,
    heading: 41.57466506958008
  }, {
    x: 2134.52294921875,
    y: 5127.32958984375,
    z: 47.586090087890625,
    heading: 41.07493209838867
  }, {
    x: 2122.82861328125,
    y: 5140.001953125,
    z: 49.215423583984375,
    heading: 41.17023849487305
  }, {
    x: 2111.558349609375,
    y: 5152.36279296875,
    z: 50.5235595703125,
    heading: 42.3946418762207
  }, {
    x: 2101.071044921875,
    y: 5163.2158203125,
    z: 51.52226257324219,
    heading: 46.47852325439453
  }, {
    x: 2093.92236328125,
    y: 5170.63818359375,
    z: 52.22397994995117,
    heading: 39.15800094604492
  }, {
    x: 2088.2880859375,
    y: 5176.5869140625,
    z: 52.8485221862793,
    heading: 43.245758056640625
  }, {
    x: 2083.858642578125,
    y: 5181.4453125,
    z: 53.471309661865234,
    heading: 39.9954948425293
  }],
  TractorWork: function TractorWork() {
    var route = this.tractorRoute;
  ...
  CowWork: function CowWork() {
    var points = this.cowPositions;
    ...
    if (this.idx < points.length) {
      ...
      var point = this.cowstoragePosition;
  ...
  // Пилот
  helicopterPositions: [{
    x: 2023.2327880859375,
    y: 4757.216796875,
    z: 41.03152084350586
  }, {
    x: 1961.8707275390625,
    y: 4731.0400390625,
    z: 49.20484924316406
  }],
  helColshape: null,
  ...
  helBlip: null,
  pilotpointIndex: 0,
  PilotWork: function PilotWork() {
    var way = this.helicopterPositions;
    var point = this.pilotpointIndex;
    var nextWay = way[point + 1];
  ...
  // Colshape Handler
  colshapeEnter: function colshapeEnter(shape) {
    var _this = this;
    ...
    if (shape == this.hayColshape) {
      ...
      setTimeout(function () {
        ...
        _this.hayStorageColshape = mp.colshapes.newSphere(2008.3099365234375, 4981.56103515625, 41.386070251464844, 1, 0);
        _this.hayStorageMarker = mp.markers["new"](1, new mp.Vector3(2008.3099365234375, 4981.56103515625, 40.386070251464844), 1, {
          color: [44, 128, 239, 150]
        });
        _this.hayStorageBlip = mp.blips["new"](1, new mp.Vector3(2008.3099365234375, 4981.56103515625, 41.386070251464844), {
          color: 3
        });
      }, 5000);
    ...
    if (shape == this.hayStorageColshape) {
      ...
      browser.execute("Farm.work.countedMoney = ".concat(countedMoney));
    } // Тракторист
    ...
    if (shape == this.cowColshape) {
      ...
      setTimeout(function () {
        ...
        _this.cowColshape.destroy();

        _this.cowMarker.destroy();

        _this.cowBlip.destroy();
    ...
    if (shape == this.cowStorageColshape) {
      ...
      setTimeout(function () {
        ...
        _this.cowStorageColshape.destroy();

        _this.cowStorageMarker.destroy();

        _this.cowStorageBlip.destroy();
      }, 5000);
    } // Пилот
...
var farmOpened = false;
mp.events.add('Farm_showWindow::CLIENT', function () {
...
mp.events.add('client::unbindFarmKey', function () {
  mp.keys.unbind(0x45, true);
...
var farmsalary = 0;
var countedMoney = 0;
mp.events.add('Farm_startWork::CLIENT', function (type, salary) {
  if (type == 1) {
...
mp.events.add('Farm_stopWork::CLIENT', function () {
  if (currentWork == 1) {
    FarmWork.hayColshape.destroy();
    FarmWork.hayMarker.destroy();
    FarmWork.hayBlip.destroy();
  } else if (currentWork == 2) {
    var ifColshape = mp.colshapes.exists(FarmWork.tractorColshape);
    var ifMarker = mp.colshapes.exists(FarmWork.tractorMarker);
    var ifBlip = mp.colshapes.exists(FarmWork.tractorBlip);
...
mp.events.add('Farm_startTractorWork::CLIENT', function () {
  FarmWork.TractorWork();
}); // ------------------------------------------[Пилот]------------------------------------------\\

mp.events.add('Farm_startPilotWork::CLIENT', function () {
  FarmWork.PilotWork();
...
mp.events.add('attached', function () {
  var vehicle = player.vehicle;
  var obj = mp.objects["new"](mp.game.joaat('prop_haybale_01'), player.position);
  console_log('hahaha');
  setTimeout(function () {
