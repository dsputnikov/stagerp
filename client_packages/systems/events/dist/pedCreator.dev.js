...
mp.events.add('pedCreator_updateData', function (data, t) {
...
mp.events.add('pedCreator_finishSync::CLIENT', function (data) {
...
function updateCharacter(t) {
  var _faceMix = parseFloat(tempData.faceMix / 100);

  var _skinMix = parseFloat(tempData.skinMix / 100);

  if (t == 1) {
    var model = tempData.gender === 0 ? 'mp_m_freemode_01' : 'mp_f_freemode_01';
    mp.events.callRemote('pedCreator_updateServer::SERVER', model);
    return;
  } //Наследственость


  player.setHeadBlendData(parseInt(tempData.father), parseInt(tempData.mother), 0, parseInt(tempData.father), parseInt(tempData.mother), 0, parseFloat(_faceMix), parseFloat(_skinMix), 0, false);

  for (var i = 0; i < tempData.structure.length; i++) {
    var value = tempData.structure[i];
    player.setFaceFeature(i, parseFloat(value));
  } // // Брови


  player.setHeadOverlay(2, tempData.hair[0], 1, 1, 1); // // Глаза

  player.setEyeColor(tempData.hair[1]); // // Причёска

  player.setComponentVariation(2, tempData.hair[2], 0, 0); // Волосы на лице

  player.setHeadOverlay(1, tempData.hair[3], 1, 1, 1); // Одежда
