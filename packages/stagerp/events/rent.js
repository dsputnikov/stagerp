...
mp.events.addCommand("rent", (player) => {
    ...
    if (player.dist(rentPosition) > radius) {
        chat.addNotify(player, 2, translations.rent_away, 7000);
        return;
    }

    if (player.getMoney() < 150) {
        chat.addNotify(player, 2, translations.rent_no_money, 7000);
        return;
    }
    ...
    chat.addNotify(player, 1, translations.rent_succes,7000)

    setTimeout(() => { 
            vehicle.destroy();
          chat.addNotify(player, 3, translations.rent_back, 7000)
      }, 900000);
});