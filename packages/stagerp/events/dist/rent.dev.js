...
mp.events.add("rentVehicle", function (player) {
  ...
  player.removeMoney(150); // Получить позицию игрока и вычислить позицию для спавна транспорта

  var playerPos = player.position;
  var spawnPos = new mp.Vector3(playerPos.x + 2, playerPos.y + 2, playerPos.z); // Создать объект Faggio и сохранить его в переменную vehicle

  var vehicle = mp.vehicles["new"](mp.joaat("faggio"), playerPos, {
    numberPlate: "RENT",
    color: [[255, 255, 255], [255, 255, 255]]
  });
  player.putIntoVehicle(vehicle, 0); // Установить владельцем транспорта текущего игрока

  vehicle.setVariable("owner", player); // Сохраняем информацию о транспорте, арендованном игроком
...
mp.events.addCommand("unrent", function (player) {
  ...
  if (vehicle && !vehicle.destroyed) {
    ...
    vehicle.destroy(); // Удаляем информацию о транспорте из объекта rentedVehicles
